<?php
session_start();

// Check if the user is authenticated and has correct permissions to view the page
if (isset($_SESSION['usertype']) && $_SESSION['usertype'] === 'A') {
    include("db.php");

    // Check if the asset_id is provided in the URL
    if (isset($_GET['asset_id'])) {
        // SQL injection prevention
        $asset_id = mysqli_real_escape_string($conn, $_GET['asset_id']);

        // Fetch asset details from the database
        $sql = "SELECT * FROM inventory_system_assets WHERE asset_id = '$asset_id'";
        $result = mysqli_query($conn, $sql);

        // Check if the asset exists
        if (mysqli_num_rows($result) > 0) {
            // display details
            $row = mysqli_fetch_assoc($result);
?>
<!DOCTYPE html>
<html lang="en">
<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <link rel="stylesheet" href="additionaldetails.css">
    <link rel="stylesheet" href="https://fonts.googleapis.com/css2?family=Lato:wght@400;700&display=swap">
    <link rel="icon" href="media/favicon.png" type="favicon.png">
    <title>Asset Details</title>
</head>
<body>
    <?php include("header.php"); ?>

    <div class="asset-details">
        <h2 class="text-heading">Asset Details</h2>

        <a href="view_assets.php" class="back-button">Back</a>

        <form action="update_asset.php" method="POST">
            <table>
                <tr>
                    <td>Asset ID:</td>
                    <td><?php echo $row['asset_id']; ?></td>
                </tr>
                <tr>
                    <td>AssetID Barcode / Qr Code:</td>
                    <td><a href="barcode_generation.php?asset_id=<?php echo $row['asset_id']; ?>" target="_blank">Barcode</a> / <a href="qr_generation.php?asset_id=<?php echo $row['asset_id']; ?>" target="_blank">Qr Code</a></td>
                    <!--refrence to the barcode generation library - https://github.com/tecnickcom/tcpdf?tab=License-1-ov-file -->
                </tr>
                <tr>
                    <td>Asset Name:</td>
                    <td><?php echo $row['asset_name']; ?></td>
                </tr>
                <tr>
                    <td>Asset Description:</td>
                    <td><?php echo $row['asset_description']; ?></td>
                </tr>
                <tr>
                    <td>Asset Location:</td>
                    <td><?php echo $row['asset_location']; ?></td>
                </tr>
                <tr>
                    <td>Asset Serial Number:</td>
                    <td><?php echo $row['asset_serialnum']; ?></td>
                </tr>
                <tr>
                    <td>Asset Group:</td>
                    <td><?php echo $row['asset_group']; ?></td>
                </tr>
                <tr>
                    <td>Date Created:</td>
                    <td><?php echo $row['date_created']; ?></td>
                </tr>
                <tr>
                    <td>Available for Borrowing:</td>
                    <td><?php echo $row['available_borrowing']; ?></td>
                </tr>
                <tr>
                    <td>Currently Being Borrowed:</td>
                    <td><?php echo $row['currently_borrowed']; ?></td>
                </tr>
                <tr>
                    <td>Borrowed By UserID:</td>
                    <td><?php
                        $borrowedUserId = $row['borrowed_by_id'];
                        if (!empty($borrowedUserId)) {
                            echo '<a href="https://w1816963.users.ecs.westminster.ac.uk/FYP/htmlphp/user_details.php?user_id=' . $borrowedUserId . '">' . $borrowedUserId . '</a>';
                        } else {
                            echo "Not borrowed by anyone";
                        }
                    ?></td>
                </tr>
            </table>
        </form>
        <button id="edit-button" type="button" onclick="editAsset()">Edit</button>

        <script>
            function editAsset() {
                // Get the asset ID
                var assetId = <?php echo json_encode($asset_id); ?>;

                // Redirect to asset_details_edit.php with asset_id as a query parameter
                window.location.href = "asset_details_edit.php?asset_id=" + assetId;
            }
        </script>
    </div>

    <?php include("footer.php"); ?>
</body>
</html>

<?php
        } else {
            // Asset not found
            echo "Asset not found.";
        }
    } else {
        // Asset ID not provided in the URL
        echo "Asset ID not provided.";
    }
    mysqli_close($conn);
} else {
    echo "<!DOCTYPE html>
    <head>
    </head>
    <body>
        <script>
            setTimeout(function() {
                window.location.href = 'accessdenied.php'; // Redirect to the login page
            }, 0);
        </script>
    </body>
    </html>";
    exit();
}
?>