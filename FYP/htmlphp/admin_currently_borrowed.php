...
<head>
    ...
    <title>Admin Currently Borrowed</title>
</head>
<body>
    <?php
    session_start();
    ...
    if (isset($_SESSION['usertype']) && $_SESSION['usertype'] === 'A') {
        ...
        $sql = "SELECT borrow_id, asset_id, user_id, borrow_time, date_borrowed, asset_returned 
                ...
                WHERE asset_returned = 'No'
                ORDER BY $sortColumn $sortOrder";
        $result = mysqli_query($conn, $sql);

        if (mysqli_num_rows($result) > 0) {
            echo "<h3 class='text-heading'>Currently Borrowed Assets, Click on the Table Row To View More Details</h3>";
           ...
           ?> <div class='button-container'>
            <a href='admin_borrow_history.php' class='borrow-history-button'>Borrow History</a>
            </div> <?php
            echo "
    ...
    function sortTable() {
        ...
        window.location.href = 'admin_currently_borrowed.php?sort=' + selectedColumn + '&order=' + selectedOrder;
    }
...
function viewAssetDetails(borrow_id) {
        window.location.href = 'admin_borrow_details.php?borrow_id=' + borrow_id;
    }
